...
1. Store contract (`src/store.ts`). The `useApp` store is module-private — never exported. Components read through selector hooks and write through plain action functions that call `useApp.setState` directly (grep `^export (function|const)` in `store.ts` for the current surface). Add new reads as selector hooks and new writes as action functions; do not expose the store.
