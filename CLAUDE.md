...
1. **Store contract (`src/store.ts`).** The `useApp` store is module-private — never exported. Components read through selector hooks (`useSections`, `useSectionTasks`, `useSectionPendingCount`) and write through plain action functions (`appendTask`, `deleteTask`, `toggleTask`, `updateTaskTitle`) that call `useApp.setState` directly. Add new reads as selector hooks and new writes as action functions; do not expose the store.
...
3. **Persistence.** Zustand `persist`, localStorage key `simple-gtd`, version 1. `partialize` persists only `sections` and `tasks` (not `sortable`). On first load, store seeds from `mockState()` — 5 GTD sections with sample tasks. Clear `simple-gtd` localStorage to reset. The `migrate` hook is commented out; bump `version` and implement for breaking state-shape changes.
...
1. **Models own state and derivations.** Views must not compute from multiple state reads — add a selector hook or store derivation instead. `useSectionPendingCount` is the pattern.
