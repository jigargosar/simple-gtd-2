...
function ViewSection({ section }: { section: Section }) {
    ...
    return (
        <div className="flex flex-col">
            <h2 className="pt-10 pb-3 text-xs font-semibold tracking-widest text-gray-500 uppercase">
                {section.title}
            </h2>
...
function ViewTask({ task: { done, id, title } }: { task: Task }) {
    return (
        <div className="group flex items-center gap-4 border-b border-gray-700 px-5 py-4 last:border-b-0">
    ...
    return (
        <button
            ...
            className={clsx(
                ...
                props.done ? 'border-gray-500 bg-gray-500' : 'border-gray-500',
            )}
        />
    )
}
...
function ViewDeleteTaskIcon(props: { onClick: () => void }) {
    return (
        <button
            onClick={props.onClick}
            className="ml-auto text-gray-600 opacity-0 transition-opacity group-hover:opacity-100 hover:text-red-400"
        >
            ✕
        </button>
    )
...
function ViewAddTask({ sectionId }: { sectionId: string }) {
    ...
    return (
        <div className="flex items-center gap-3">
            ...
            />
            {value.trim() && (
                <button onClick={submit} className="text-sm text-gray-400 hover:text-gray-100">
                    ↵
                </button>
            )}
        </div>
